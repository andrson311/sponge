...
void MidpointDispTerrain::CreatMidpointDisplacement(int TerrainSize, float Roughness, float MinHeight, float MaxHeight)
{
    if (Roughness < 0.0f)
    {
        printf("Roughness must be positive - %f\n", Roughness);
        exit(0);
    }
...
void MidpointDispTerrain::SquareStep(int RectSize, float CurHeight)
{
    ...
    for (int y = 0; y < m_terrainSize; y += RectSize)
    {
        for (int x = 0; x < m_terrainSize; x += RectSize)
        {
            ...
            float PrevYCenter = m_heightMap.Get(mid_x, prev_mid_x);
